...
class Draw:
    ...
    def drawHeldPiece(self, board):
        ...
        tempPiece = Tetra(board.heldPiece.shape, 0, board.heldPiece.colour)
        if shape in ("S","Z","I"):
    ...
    def drawTetromino(self, tetra):
        pygame.draw.polygon(self.screen, tetra.colour, self.getScaledCoords(tetra.vertexCoords))
        pygame.draw.polygon(self.screen, "Black", self.getScaledCoords(tetra.vertexCoords), self.pieceOutline)

    def updateDisplay(self, board, tetra):
        self.board = board
        self.tetra = tetra
    ...
    def refreshScreen(self, board, tetra):
        self.screen.fill("Black")
        self.drawBackground(board)
        self.drawGridPieces(board)
        self.drawTetromino(tetra)
